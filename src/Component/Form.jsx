...
function Form() {
  ...
  const Register = async () => {
    try {
      const user = await axios.post("https://skillshare-backend-070t.onrender.com/user/auth/signup",{
        name: name,
        email: email,
        password: password
      })
      {!user ? <Loader/> :
      alert(`register successfully ${user.data.newUser.name}`);
      navigate("/login");
    }
    } catch (err) {
      alert(err.message);
    ...
    <>
      <div className="container" id="ab8">
        <div id="login_logo">
          ...
          <span className="text-white bg-success mx-2 p-1 ab9">
            {" "}
